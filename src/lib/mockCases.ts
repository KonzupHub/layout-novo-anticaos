...
export const getTipoBadge = (tipo: Case['tipo']) => {
  const badges = {
    ...
    extravio: { label: 'Extravio', color: 'bg-amber-100 text-amber-800 border-amber-200' }
  };
  return badges[tipo] || { label: tipo, color: 'bg-gray-100 text-gray-800 border-gray-200' };
};
